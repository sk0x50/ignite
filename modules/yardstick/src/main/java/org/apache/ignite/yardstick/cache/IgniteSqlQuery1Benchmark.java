 ...
 */
public class IgniteSqlQuery1Benchmark extends IgniteCacheAbstractBenchmark<Integer, OneIndex> {
    /** */
    ...
    @Override protected void loadCacheData(String cacheName) {
        try (IgniteDataStreamer<Integer, OneIndex> dataLdr = ignite().dataStreamer(cacheName)) {
            for (int i = 0; i < args.range(); i++) {
                if (i % 100 == 0 && Thread.currentThread().isInterrupted())
                    break;

                dataLdr.addData(i, new OneIndex(i));
    ...
    @Override public boolean test(Map<Object, Object> ctx) throws Exception {
        ...
        Collection<Cache.Entry<Integer, OneIndex>> entries = executeQuery(field, minValue, maxValue);
...
//        }
     ...
     */
    private Collection<Cache.Entry<Integer, OneIndex>> executeQuery(String fieldName, double minValue, double maxValue) throws Exception {
        IgniteCache<Integer, OneIndex> cache = cacheForOperation(true);

        SqlQuery qry = new SqlQuery(OneIndex.class, fieldName + " >= ? and " + fieldName + " <= ?");
